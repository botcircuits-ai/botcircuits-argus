...
def add_workflow_subparser(subparsers: argparse._SubParsersAction) -> None:
    ...
    gen_p.add_argument(
        "--subdir", dest="subdir", default=None,
        help="Write the generated source under this sub-directory of the "
             "workflows dir (e.g. 'coding' for per-task coding workflows). "
...
def _cmd_generate(args: argparse.Namespace) -> int:
    ...
    # Write the SOURCE file. Top-level sources never clobber an existing
    # file (a hand-authored workflow) — the name must be distinct. Sources
    # written under a --subdir (the coding pipeline regenerating a per-task
    # workflow) DO overwrite: re-running the same coding task should replace
    # its generated workflow, not fail.
    workflows_dir = _resolve_workflows_dir()
    subdir = getattr(args, "subdir", None)
    dest_dir = workflows_dir / subdir if subdir else workflows_dir
    try:
        dest_dir.mkdir(parents=True, exist_ok=True)
    except OSError as e:
        out(C.red(f"[workflow] cannot create {dest_dir}: {e}"))
        return 1
    dest = dest_dir / f"{name}.json"
    if dest.exists() and not subdir:
        out(C.red(
...
def _cmd_build(args: argparse.Namespace) -> int:
    ...
    # Mirror the source's location UNDER the workflows dir into the build
    # dir, so a source at `coding/<name>.json` builds to
    # `.build/coding/<name>.json` (and a top-level source stays flat).
    try:
        rel = source_path.relative_to(_resolve_workflows_dir())
    except ValueError:
    ...
    try:
        ...
        out(C.red(
            f"[workflow] failed to create build dir {build_path.parent}: "
            f"{type(e).__name__}: {e}"
        ))
        return 1
...
def _locate_workflow_file(workflow_name: str) -> tuple[Path, dict]:
    ...
    # Direct hit: top-level first, then the coding/ subdir (per-task
    # coding workflows generated by the pipeline live there).
    from botcircuits.agent.workflow.local import CODING_SUBDIR
    for direct in (directory / f"{workflow_name}.json",
                   directory / CODING_SUBDIR / f"{workflow_name}.json"):
        if direct.exists():
            return direct, _load_json(direct)

    search = sorted(directory.glob("*.json"))
    coding_src = directory / CODING_SUBDIR
    if coding_src.is_dir():
        search.extend(sorted(coding_src.glob("*.json")))
    for path in search:
        try:
