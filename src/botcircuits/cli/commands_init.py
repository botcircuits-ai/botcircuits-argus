...
from botcircuits.runtime.detect import BOTCIRCUITS, CLAUDE_CODE, CODEX, HERMES, OPENCLAW

#: Host agent runtimes selectable via `--runtime`. `botcircuits` pins the
#: native in-process agent — its workflow tools (`build_workflow` and each
...
RUNTIME_CHOICES = (BOTCIRCUITS, CLAUDE_CODE, HERMES)
...
def run_init_command(args: argparse.Namespace) -> int:
    ...
    if runtime == BOTCIRCUITS:
        out(C.dim(
            "  (runtime 'botcircuits' runs natively — build_workflow and "
