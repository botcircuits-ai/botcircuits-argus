...
# Sub-directory (under both the source dir and the build dir) that holds
# per-task CODING workflows generated by the coding pipeline's
# `author_coding_workflow` step. Kept separate from hand-authored
# workflows so they don't crowd the top-level list; the loader still
# discovers them and they run by name like any other workflow.
CODING_SUBDIR = "coding"


def _built_workflow_paths(build_dir: Path) -> list[Path]:
    """Every runnable workflow JSON under `build_dir` — the flat top-level
    files plus the per-task coding workflows nested under `coding/`.

    A single place both the discovery (`fetch_workflows`) and by-name
    lookup (`_load_workflow_record`) enumerate, so they never disagree on
    which files count as runnable."""
    if not build_dir.is_dir():
        return []
    paths = sorted(build_dir.glob("*.json"))
    coding_dir = build_dir / CODING_SUBDIR
    if coding_dir.is_dir():
        paths.extend(sorted(coding_dir.glob("*.json")))
    return paths
...
async def fetch_workflows() -> list[dict]:
    ...
    if source_dir.is_dir():
        built_stems = {p.stem for p in _built_workflow_paths(build_dir)}
        # Both the flat sources and the nested coding/ sources; a missing
        # build for either gets the same "run workflow build" nudge.
        raw_sources = sorted(source_dir.glob("*.json"))
        coding_src = source_dir / CODING_SUBDIR
        if coding_src.is_dir():
            raw_sources.extend(sorted(coding_src.glob("*.json")))
        for raw_path in raw_sources:
            if raw_path.stem not in built_stems:
                print(
                    f"[workflow] skipping {raw_path}: no build. Run "
    ...
    records: list[dict] = []
    for path in _built_workflow_paths(build_dir):
        try:
...
def _load_workflow_record(name: str) -> dict:
    ...
    # Direct hit: top-level first, then the coding/ subdir — a generated
    # coding workflow lives under coding/ but runs by its plain name.
    for direct in (directory / f"{name}.json",
                   directory / CODING_SUBDIR / f"{name}.json"):
        if direct.exists():
    ...
    for path in _built_workflow_paths(directory):
        try:
            with path.open("r", encoding="utf-8") as f:
                record = json.load(f)
